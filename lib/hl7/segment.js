...
segment.prototype.getArrayFieldComponent = function(fieldIndex,subFieldIndex, componentIndex, subComponentIndex) {
  if (this.fields.length >= fieldIndex) {
    var values = this.fields[fieldIndex - 1].value;

        if(values.length === 1 && subFieldIndex === 1 ){
          var components = values[subFieldIndex-1];
          if (components.length >= componentIndex) {
            var component = components[componentIndex - 1]
            if (subComponentIndex) {
              if (component.value[0].length >= subComponentIndex) {
        ...
        if(values.length>=subFieldIndex)
        {
          var components = values[subFieldIndex-1].value[0];
          if (components.length >= componentIndex) {
            var component = components[componentIndex - 1]
            if (subComponentIndex) {
              if (component.value[0].length >= subComponentIndex) {
